...
async fn test_multiplexer_registers_waiter() {
    ...
    let rx = multiplexer.register_waiter("test-1".to_string()).await;
...
async fn test_multiplexer_routes_correct_response() {
    ...
    let rx1 = multiplexer.register_waiter("request-1".to_string()).await;
    let rx2 = multiplexer.register_waiter("request-2".to_string()).await;
